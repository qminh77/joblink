...
function buildCases(uc, module, title, actor, route, codeEntry, requiredFields) {
  const fields = requiredFields.join(", ")
  const prefix = modulePrefixes[module]
  const ucNum = uc.replace('-', '') // e.g. UC01
  const [mainRule, alternateRule, stateRule, integrationRule, uiRule] = businessRulesFor(uc, title)

  return [
    {
      id: `TC-${prefix}-${ucNum}-01`,
      kind: "functional",
      title: `${title} - luồng chính`,
      preconditions: [`Actor ${actor} matches the SRS actor for ${uc}`, `Route ${route} is reachable`],
      steps: [
        `Mở ${route}`,
        `Thực hiện ${title} với dữ liệu: ${fields}`,
      ],
      expected: [
        mainRule,
        "Luồng chính thành công",
      ],
      dataChecks: [`${fields} are persisted or returned correctly`],
    },
    {
      id: `TC-${prefix}-${ucNum}-02`,
      kind: "required_fields",
      title: `${title} - trường bắt buộc`,
      preconditions: [`Actor ${actor} can reach ${route}`],
      steps: [
        `Bỏ trống từng trường bắt buộc hoặc xác nhận bắt buộc rồi gửi form/thao tác. (${fields})`
      ],
      expected: [
        "Hệ thống chặn lưu.",
        "Báo lỗi đúng trường.",
      ...
      dataChecks: [`Required fields are enforced: ${fields}`],
    },
    {
      id: `TC-${prefix}-${ucNum}-03`,
      kind: "boundary",
      title: `${title} - dữ liệu biên và sai định dạng`,
      preconditions: [`Actor ${actor} can reach ${route}`],
      steps: [
        `Nhập dữ liệu min/max, quá giới hạn, sai định dạng hoặc file sai loại/dung lượng nếu UC có file. (${fields})`
      ],
      expected: [
      ...
      dataChecks: ["Schema or service validation rejects invalid payload"],
    },
    {
      id: `TC-${prefix}-${ucNum}-04`,
      kind: "auth",
      title: `${title} - truy cập và phiên đăng nhập`,
      preconditions: ["Trạng thái đăng nhập được thiết lập"],
      steps: [
      ...
      dataChecks: ["Session guard is evaluated"],
    },
    {
      id: `TC-${prefix}-${ucNum}-05`,
      kind: "permission",
      title: `${title} - phân quyền và trạng thái nghiệp vụ`,
      preconditions: ["Use an account with different role or ownership"],
      steps: [
      ...
      dataChecks: ["Role/ownership/status guard is checked"],
    },
    {
      id: `TC-${prefix}-${ucNum}-06`,
      kind: "alternative",
      title: `${title} - ngoại lệ nghiệp vụ`,
      preconditions: [`Actor ${actor} can start ${title}`],
      steps: [
        `Thực hiện ngoại lệ nghiệp vụ cho ${title}`
      ],
      expected: [
        alternateRule
      ],
      dataChecks: ["No partial mutation is committed"],
    },
    {
      id: `TC-${prefix}-${ucNum}-07`,
      kind: "state_transition",
      title: `${title} - chuyển trạng thái`,
      ...
      dataChecks: ["Old and new state are not both active"],
    },
    {
      id: `TC-${prefix}-${ucNum}-08`,
      kind: "side_effect",
      title: `${title} - tác động liên quan`,
      ...
      dataChecks: ["Expected side effects are present only once"],
    },
    {
      id: `TC-${prefix}-${ucNum}-09`,
      kind: "integration",
      title: `${title} - tích hợp dữ liệu`,
      ...
      dataChecks: ["Related cache/revalidation is consistent"],
    },
    {
      id: `TC-${prefix}-${ucNum}-10`,
      kind: "ui_feedback",
      title: `${title} - phản hồi giao diện`,
      ...
      dataChecks: ["Visible state matches action/query result"],
    },
    {
      id: `TC-${prefix}-${ucNum}-11`,
      kind: "regression",
      title: `${title} - hồi quy sau sửa lỗi`,
      preconditions: ["Đã có lỗi được ghi nhận và sửa chữa"],
      steps: [
      ...
      dataChecks: ["Regression test passed"],
    }
  ]
}
