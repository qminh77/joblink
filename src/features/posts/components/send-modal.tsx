...
export function SendModal({ post, onClose }: Props) {
  return (
    <AnimatePresence>
      {post ? (
        <SendModalInner key={post.id} post={post} onClose={onClose} />
      ) : null}
    </AnimatePresence>
  )
}

function SendModalInner({
  post,
  onClose,
}: {
  post: FeedPost
  ...
  async function handleSend(userId: number) {
    ...
    try {
      ...
      const content = trimmedNote ? `${trimmedNote}\n${url}` : url
