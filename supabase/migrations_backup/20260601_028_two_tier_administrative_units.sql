-- =============================================================================
-- JOBLINK MIGRATION 20260601_028 — ĐƠN VỊ HÀNH CHÍNH 2 CẤP (2025)
-- =============================================================================
-- Chuyển mô hình địa danh cũ (tỉnh + quận/huyện) sang mô hình hành chính 2 cấp
-- hiệu lực 01/07/2025: TỈNH/THÀNH PHỐ  →  XÃ/PHƯỜNG/ĐẶC KHU.
--   • Bảng `districts` (quận/huyện) đổi tên thành `wards` (cấp xã).
--   • Cột district_id → ward_id ở member_profiles / company_profiles / jobs.
--   • Tạo lại các view + RPC tham chiếu (plpgsql late-binding nên phải recreate).
--   • Seed lại 34 tỉnh/thành + 3.320 đơn vị cấp xã (687 phường, 2.620 xã, 13 đặc
--     khu). Nguồn dữ liệu công khai zuydd/vn-geo (06/2025).
--
-- Migration idempotent: dùng guard IF EXISTS nên chạy lại an toàn. DÀNH CHO DB
-- ĐÃ TỒN TẠI (giữ dữ liệu). Lưu ý: FK province_id/ward_id dùng ON DELETE SET
-- NULL, nên việc seed lại danh mục sẽ đặt NULL các tham chiếu địa danh CŨ.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Đổi tên bảng + cột (cấu trúc) — guarded để idempotent
-- -----------------------------------------------------------------------------
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema='public' AND table_name='districts') THEN
    ALTER TABLE public.districts RENAME TO wards;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='member_profiles' AND column_name='district_id') THEN
    ALTER TABLE public.member_profiles RENAME COLUMN district_id TO ward_id;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_profiles' AND column_name='district_id') THEN
    ALTER TABLE public.company_profiles RENAME COLUMN district_id TO ward_id;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='jobs' AND column_name='district_id') THEN
    ALTER TABLE public.jobs RENAME COLUMN district_id TO ward_id;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='uk_districts_code') THEN
    ALTER TABLE public.wards RENAME CONSTRAINT uk_districts_code TO uk_wards_code;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_district_province') THEN
    ALTER TABLE public.wards RENAME CONSTRAINT fk_district_province TO fk_ward_province;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_member_profile_district') THEN
    ALTER TABLE public.member_profiles RENAME CONSTRAINT fk_member_profile_district TO fk_member_profile_ward;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_company_profile_district') THEN
    ALTER TABLE public.company_profiles RENAME CONSTRAINT fk_company_profile_district TO fk_company_profile_ward;
  END IF;
END $mig$;
DO $mig$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_job_district') THEN
    ALTER TABLE public.jobs RENAME CONSTRAINT fk_job_district TO fk_job_ward;
  END IF;
END $mig$;

-- Index (ALTER INDEX hỗ trợ IF EXISTS)
ALTER INDEX IF EXISTS public.idx_districts_province RENAME TO idx_wards_province;
ALTER INDEX IF EXISTS public.idx_districts_active RENAME TO idx_wards_active;
ALTER INDEX IF EXISTS public.idx_member_profiles_district RENAME TO idx_member_profiles_ward;
ALTER INDEX IF EXISTS public.idx_company_profiles_district RENAME TO idx_company_profiles_ward;
ALTER INDEX IF EXISTS public.idx_jobs_district RENAME TO idx_jobs_ward;

-- Trigger updated_at (DROP+CREATE cho chắc, không phụ thuộc phiên bản PG)
DROP TRIGGER IF EXISTS trg_districts_set_updated_at ON public.wards;
...
-- -----------------------------------------------------------------------------
-- 2. Tạo lại VIEW tham chiếu ward (đổi alias district_name → ward_name nên phải
--    DROP trước khi tạo lại).
-- -----------------------------------------------------------------------------
DROP VIEW IF EXISTS public.v_active_members;
DROP VIEW IF EXISTS public.v_verified_companies;
DROP VIEW IF EXISTS public.v_active_jobs;
...
-- -----------------------------------------------------------------------------
-- 3. Tạo lại các RPC tham chiếu ward (body-only → CREATE OR REPLACE).
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. create_job / update_job đổi TÊN tham số (p_district_id → p_ward_id) nên
--    không thể CREATE OR REPLACE; drop mọi overload theo tên rồi tạo lại + GRANT.
-- -----------------------------------------------------------------------------
DO $mig$
DECLARE r record;
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
            WHERE proname IN ('create_job','update_job')
...
-- -----------------------------------------------------------------------------
-- 5. Seed lại danh mục đơn vị hành chính 2025 (34 tỉnh + 3.320 cấp xã).
-- -----------------------------------------------------------------------------
