...
-- Charset: UTF8
-- 
-- ⮕ FILE DUY NHẤT: gộp từ TOÀN BỘ supabase/migrations (đến 20260619_050).
-- ⮕ ĐÃ LOẠI các bảng không dùng: security_logs.
...
SELECT u.id,
       COALESCE(au.raw_user_meta_data->>'company_name', split_part(u.email, '@', 1)),
       regexp_replace(
         lower(COALESCE(au.raw_user_meta_data->>'company_name', split_part(u.email, '@', 1))),
         '[^a-z0-9]+', '-', 'g'
...
CREATE POLICY users_select_own ON public.users FOR SELECT USING (auth_id::text = auth.uid()::text);
CREATE POLICY users_update_own ON public.users FOR UPDATE USING (auth_id::text = auth.uid()::text);

ALTER TABLE IF EXISTS public.member_profiles            ENABLE ROW LEVEL SECURITY;
CREATE POLICY member_profiles_select_all ON public.member_profiles FOR SELECT USING (true);
CREATE POLICY member_profiles_update_own ON public.member_profiles FOR UPDATE USING (user_id = public.auth_user_id());

ALTER TABLE IF EXISTS public.company_profiles           ENABLE ROW LEVEL SECURITY;
CREATE POLICY company_profiles_select_all ON public.company_profiles FOR SELECT USING (true);
CREATE POLICY company_profiles_update_own ON public.company_profiles FOR UPDATE USING (user_id = public.auth_user_id());

ALTER TABLE IF EXISTS public.jobs                       ENABLE ROW LEVEL SECURITY;
CREATE POLICY jobs_select_visible ON public.jobs FOR SELECT USING (status = 'active' OR company_user_id = public.auth_user_id());
CREATE POLICY jobs_insert_own ON public.jobs FOR INSERT WITH CHECK (company_user_id = public.auth_user_id());
CREATE POLICY jobs_update_own ON public.jobs FOR UPDATE USING (company_user_id = public.auth_user_id());
CREATE POLICY jobs_delete_own ON public.jobs FOR DELETE USING (company_user_id = public.auth_user_id());

ALTER TABLE IF EXISTS public.job_applications           ENABLE ROW LEVEL SECURITY;
CREATE POLICY job_applications_select_visible ON public.job_applications FOR SELECT USING (applicant_id = public.auth_user_id() OR job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id()));
CREATE POLICY job_applications_insert_own ON public.job_applications FOR INSERT WITH CHECK (applicant_id = public.auth_user_id());
CREATE POLICY job_applications_update_own ON public.job_applications FOR UPDATE USING (applicant_id = public.auth_user_id() OR job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id()));

ALTER TABLE IF EXISTS public.application_status_history ENABLE ROW LEVEL SECURITY;
CREATE POLICY status_history_select_visible ON public.application_status_history FOR SELECT USING (application_id IN (SELECT id FROM public.job_applications WHERE applicant_id = public.auth_user_id() OR job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id())));
CREATE POLICY status_history_insert_own ON public.application_status_history FOR INSERT WITH CHECK (application_id IN (SELECT id FROM public.job_applications WHERE applicant_id = public.auth_user_id() OR job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id())));

ALTER TABLE IF EXISTS public.interview_schedules        ENABLE ROW LEVEL SECURITY;
CREATE POLICY interview_select_visible ON public.interview_schedules FOR SELECT USING (application_id IN (SELECT id FROM public.job_applications WHERE applicant_id = public.auth_user_id() OR job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id())));
CREATE POLICY interview_insert_own ON public.interview_schedules FOR INSERT WITH CHECK (application_id IN (SELECT id FROM public.job_applications WHERE job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id())));
CREATE POLICY interview_update_own ON public.interview_schedules FOR UPDATE USING (application_id IN (SELECT id FROM public.job_applications WHERE job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id())));

ALTER TABLE IF EXISTS public.saved_jobs                 ENABLE ROW LEVEL SECURITY;
CREATE POLICY saved_jobs_select_own ON public.saved_jobs FOR SELECT USING (user_id = public.auth_user_id());
CREATE POLICY saved_jobs_insert_own ON public.saved_jobs FOR INSERT WITH CHECK (user_id = public.auth_user_id());
CREATE POLICY saved_jobs_delete_own ON public.saved_jobs FOR DELETE USING (user_id = public.auth_user_id());
...
CREATE POLICY job_skills_insert_own ON public.job_skills FOR INSERT WITH CHECK (job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id()));
CREATE POLICY job_skills_delete_own ON public.job_skills FOR DELETE USING (job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id()));

ALTER TABLE IF EXISTS public.job_view_logs              ENABLE ROW LEVEL SECURITY;
CREATE POLICY job_view_logs_insert_all ON public.job_view_logs FOR INSERT WITH CHECK (true);
CREATE POLICY job_view_logs_select_own ON public.job_view_logs FOR SELECT USING (job_id IN (SELECT id FROM public.jobs WHERE company_user_id = public.auth_user_id()));
...
CREATE POLICY messages_update_own ON public.messages
  FOR UPDATE USING (sender_id = public.auth_user_id() AND deleted_at IS NULL) WITH CHECK (sender_id = public.auth_user_id());
CREATE POLICY messages_update_read ON public.messages
  FOR UPDATE USING (public.is_my_conversation(conversation_id)) WITH CHECK (public.is_my_conversation(conversation_id));
...
CREATE POLICY posts_select_visible ON public.posts
  FOR SELECT USING (
    ...
    AND (
      ...
      OR (visibility = 'connections' AND public.are_connected(public.auth_user_id(), author_id))
    )
  );
CREATE POLICY posts_insert_own ON public.posts
  FOR INSERT WITH CHECK (author_id = public.auth_user_id());
CREATE POLICY posts_update_own ON public.posts
  FOR UPDATE USING (author_id = public.auth_user_id()) WITH CHECK (author_id = public.auth_user_id());
...
CREATE POLICY poll_options_insert_own ON public.poll_options
  FOR INSERT WITH CHECK (
    post_id IN (SELECT p.id FROM public.posts p WHERE p.author_id = public.auth_user_id())
  );
...
BEGIN
    ...
    IF v_is_visible THEN
        ...
        SELECT COALESCE(jsonb_agg(jsonb_build_object('id', s.id, 'name', s.name)), '[]'::jsonb)
          INTO v_skills FROM public.member_skills ms JOIN public.skills s ON s.id = ms.skill_id
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object('id', s.id, 'name', s.name)), '[]'::jsonb)
      INTO v_skills FROM public.member_skills ms JOIN public.skills s ON s.id = ms.skill_id WHERE ms.user_id = v_me;
...
CREATE OR REPLACE FUNCTION public.create_job(
    ...
    p_status TEXT, p_expires_at TIMESTAMPTZ, p_skills TEXT[]
)
RETURNS JSONB LANGUAGE plpgsql SECURITY INVOKER VOLATILE AS $$
DECLARE
    v_me BIGINT; v_role TEXT; v_status TEXT; v_job_id BIGINT;
    v_skill_name TEXT; v_skill_id BIGINT;
BEGIN
...
GRANT EXECUTE ON FUNCTION public.create_job(
    TEXT, TEXT, TEXT, BIGINT, BIGINT, BIGINT, BIGINT, BOOLEAN, BIGINT, BIGINT, BIGINT, TEXT, TIMESTAMPTZ, TEXT[]
) TO authenticated;
